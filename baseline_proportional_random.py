"""
Baseline 3: Proportional (1-SSIM) allocation + Random weight selection
- Budget allocation : each layer gets budget ∝ (1 - SSIM_i), normalized across layers
- Weight selection  : randomly sample from pruned (mask=0) weights
"""
...
# ── Proportional (1-SSIM) allocation ─────────────────────────────────────────
def allocate_proportional(target_layers, ssim_dict, layer_capacities, total_budget):
    weights   = [max(1.0 - ssim_dict.get(l, 0.5), 1e-6) for l in target_layers]
    total_w   = sum(weights)
    raw       = [total_budget * w / total_w for w in weights]
    counts    = [min(max(round(r), 0), c) for r, c in zip(raw, layer_capacities)]
    # redistribute any leftover due to rounding / capacity clamping
    remaining = total_budget - sum(counts)
    if remaining > 0:
        order = sorted(range(len(target_layers)),
                       key=lambda i: layer_capacities[i] - counts[i], reverse=True)
        for i in order:
            if remaining <= 0:
                break
            add = min(layer_capacities[i] - counts[i], remaining)
            if add > 0:
                counts[i]  += add
                remaining  -= add
    return {l: c for l, c in zip(target_layers, counts) if c > 0}


# ── Random weight selection ───────────────────────────────────────────────────
@torch.no_grad()
def apply_random_regrowth(model, layer_name, num_weights, device):
    module = dict(model.named_modules()).get(layer_name)
    if module is None or not hasattr(module, 'weight_mask'):
        return 0
    mask    = module.weight_mask
    pruned  = (mask == 0).nonzero(as_tuple=False)
    if len(pruned) == 0:
        return 0
    k       = min(num_weights, len(pruned))
    chosen  = pruned[torch.randperm(len(pruned))[:k]]
...
def main():
    ...
    parser.add_argument('--save_dir',        type=str,   default='./baselines/proportional_random')
    ...
    print(f"[proportional_random] Device={device}  seed={args.seed}")
    ...
    run = None if args.no_wandb else wandb.init(
        ...
        name=f"proportional_random_{args.m_name}_s{args.seed}",
        config=vars(args))
    ...
    for it in range(args.num_iters):
        ...
        allocation   = allocate_proportional(target_layers, ssim_dict, layer_caps, total_budget)
        print(f"  Budget={total_budget}  Allocation={allocation}")

        model_copy = create_copy(current_model, args.m_name, device)
        regrown = sum(apply_random_regrowth(model_copy, l, n, device)
                      for l, n in allocation.items())
