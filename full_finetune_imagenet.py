...
parser.add_argument('--model_path', type=str,
                    default='./rl_saliency_checkpoints/vgg16TinyImageNet/oneshot/0.94/regrown_model.pth')
...
def full_finetune(model, train_loader, test_loader, device,
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 50 == 0:
            torch.save(model.state_dict(), f'./rl_saliency_checkpoints/vgg16TinyImageNet/oneshot/0.94fullfinetune/epoch{epoch}.pth')
...
final_acc = full_finetune(
    ...
    epochs=400,
    lr=0.0003,
    save_path=f"./rl_saliency_checkpoints/vgg16TinyImageNet/oneshot/0.94fullfinetune/final_model_{args.sparsity}.pth",
    patience=50,
)
print(f"\n{'=' * 60}")
