...
import numpy as np
from tools import *
from tools.utils import load_model, remove_pruning_reparam
import copy
...
#         torch.save(model.state_dict(),os.path.join(DICT_PATH,'unpruned_with_mask.pth'))
...
pretrained_net, best_acc, _ = load_model(model, "/home/j/junchen/pytorch-cifar/checkpoint")
...
for it in range(args.iter_start,args.iter_end+1):
    ...
    torch.save(results_to_save,BASE_PATH+f'/{args.pruner}.tsr')

target_folder = './ckpt_after_prune'
os.makedirs(target_folder, exist_ok=True)
target_path_mask = os.path.join(target_folder, f'resnet_pruned_finetuned_with_mask_s{amount_per_it}.pth')
torch.save(model.state_dict(), target_path_mask)

remove_pruning_reparam(model)
target_path_final = os.path.join(target_folder, f'resnet_pruned_finetuned_with_finals{amount_per_it}.pth')
torch.save(model.state_dict(), target_path_final)
