"""
RL-based Iterative Regrowth — Random Layer Selection
=====================================================
Same as rl_saliency_regrowth_iterative_new.py but layer selection is random
instead of SSIM-guided. Each outer iteration, layers are randomly sampled
from all masked layers without replacement.

Key differences vs SSIM version:
  - No pretrained model needed for SSIM (still loaded for saliency)
  - --num_random_layers controls how many layers enter the RL search space
    (default: all masked layers)
  - Layer order is reshuffled each outer iteration
"""
...
def random_layer_selection(all_masked_layers, num_layers=None):
    """
    Randomly shuffle all masked layers and return the first num_layers.
    If num_layers is None or >= total, return all layers in random order.
    """
    layers = list(all_masked_layers)
    random.shuffle(layers)
    if num_layers is not None and num_layers < len(layers):
        layers = layers[:num_layers]
    print(f"\n  ── Random Layer Selection ──")
    print(f"  Selected {len(layers)}/{len(all_masked_layers)} layers:")
...
class RegrowthPolicyGradient:
    def __init__(self, config, model_sparse, saliency_dict,
        ...
        self.checkpoint_dir = config.get('checkpoint_dir', './rl_random_checkpoints')
    ...
    def mini_finetune(self, model, epochs=None):
    ...
    def play_episode(self, epoch):
        ...
        for p_idx, (lname, orig_idx) in enumerate(self.layer_priority):
            ...
            # deduplicate: keep first occurrence of each count
            seen_vals: set = set()
            dedup_mask = torch.zeros(self.ALLOC_SPACE, dtype=torch.bool, device=self.DEVICE)
            for _i, _v in enumerate(c_opts.tolist()):
                if _v not in seen_vals:
                    seen_vals.add(_v)
                    dedup_mask[_i] = True

            feasible = (c_opts <= remaining) & dedup_mask
...
def main():
    ...
    parser.add_argument('--num_random_layers', type=int, default=None,
                        help='How many layers to sample each iteration. '
                             'Default: all masked layers.')
    ...
    # Misc
    parser.add_argument('--save_dir',     type=str, default='./rl_random_checkpoints')
    ...
    run = None if args.no_wandb else wandb.init(
        project="ICCAD_random_layer_iterative",
        name=(f"{args.m_name}_{args.start_sparsity:.4f}_rand"
              f"{'_top'+str(args.num_random_layers) if args.num_random_layers else '_all'}"),
        config=vars(args) | {"total_weights": total_weights},
    )

    # Pretrained model (for saliency only)
    ...
    # Enumerate all masked layers from initial checkpoint
    ...
    for iter_idx in range(args.resume_iter, args.num_iters):
        ...
        # Random layer selection (reshuffled each iteration)
        target_layers = random_layer_selection(all_masked_layers, args.num_random_layers)
