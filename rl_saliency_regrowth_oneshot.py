...
class PolicyGradient:
    ...
    def run_episode(self):
        ...
        self.mini_finetune(model, self.config.get('mini_finetune_epochs', 50))
...
def main():
    ...
    # Load pruned checkpoint and target layers based on model
    ...
    # Calculate layer capacities and target regrowth
    layer_capacities = []
    for layer_name in target_layers:
        module_99 = dict(model_99.named_modules())[layer_name]
        if hasattr(module_99, 'weight_mask'):
            capacity = (module_99.weight_mask == 0).sum().item()
    ...
    config = {
        ...
        'wandb_name': args.wandb_name or f"{args.m_name}_{args.method}_regrowth",
    }
    ...
    # Evaluate before
    before_acc = pg.evaluate(model_99, full=True)
    print(f"\nBefore regrowth: {before_acc:.2f}%")
        ...
        if num_weights > 0:
            ...
            if saliency is not None:
                SaliencyRegrowth.apply(
                    model_99, layer_name, saliency, num_weights,
                    args.init_strategy, device
                )

    # Evaluate after regrowth
    after_acc = pg.evaluate(model_99, full=True)
    print(f"After regrowth: {after_acc:.2f}% (+{after_acc - before_acc:.2f}%)")

    # Final finetuning
    final_acc, final_state = full_finetune(model_99, train_loader, test_loader, device, config)
    ...
    print(f"After regrowth: {after_acc:.2f}%")
    print(f"After finetuning: {final_acc:.2f}%")
    print(f"Total improvement: {final_acc - before_acc:+.2f}%")
    print("=" * 70)
