...
class RegrowthPolicyGradient:
    ...
    def __init__(self, config, model_pretrained, model_99,
        ...
        print(f"  min_epochs       : {self.min_epochs}")
        print(f"  reward_std_thresh: {self.reward_std_threshold}")
        print(f"  entropy_thresh   : {self.min_entropy_threshold}")
        print(f"  reward_window    : {self.reward_window_size}")
        if self.acc_baseline is not None:
            print(f"\nAccuracy baseline : {self.acc_baseline * 100:.2f}%  "
                  f"(model saved whenever episode acc exceeds this)")
    ...
    def solve_environment(self, resume_from=None):
        """Main RL training loop with early stopping"""
        ...
        while epoch < self.NUM_EPOCHS:
            ...
            (episode_weighted_log_prob,
             ...
             allocation,
             sparsity,
             regrow_indices) = self.play_episode(solve_start_time, epoch)

            # Track reward history for early stopping
            ...
            reward_std = float(np.std(list(reward_window))) if len(reward_window) > 1 else float('inf')
            current_entropy = entropy.item()
            should_stop = False

            # Condition 1: no improvement for `patience` epochs
            if epochs_no_improve >= self.early_stop_patience:
                should_stop = True
                stop_reason = (f"No improvement for {self.early_stop_patience} epochs "
                               f"(last improved at epoch {best_reward_epoch + 1})")

            # Condition 2: reward window std below threshold (only after min_epochs)
            elif epoch >= self.min_epochs and len(reward_window) >= self.reward_window_size:
            ...
            if should_stop:
                print(f"\n>>> Early stopping triggered at epoch {epoch + 1}: {stop_reason}")
                break
    ...
    def _save_baseline_model(self, epoch, accuracy, model, allocation):
        ...
        print(f"  ✓ Baseline exceeded! acc={accuracy * 100:.2f}% > "
              f"{self.acc_baseline * 100:.2f}% → saved: {save_path}")
...
def main():
    ...
    parser.add_argument('--reward_std_threshold', type=float, default=0.002,
                        help='Stop if reward window std < this value (policy stable)')
    parser.add_argument('--min_entropy_threshold', type=float, default=0.05,
                        help='Stop if policy entropy < this value (policy collapsed)')
    parser.add_argument('--reward_window_size', type=int, default=20,
                        help='Rolling window size for reward std computation')
    ...
    if args.acc_baseline is not None:
        print(f"Baseline threshold: {args.acc_baseline * 100:.2f}%  "
              f"(all exceeding models saved to {args.save_dir})")
    print(f"{'=' * 70}\n")
