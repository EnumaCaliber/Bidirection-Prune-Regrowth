"""
Baseline 1: Greedy SSIM allocation + Random weight selection
...
def main():
    ...
    parser.add_argument('--save_dir',        type=str,   default='./baselines/greedy_random')
    ...
    print(f"[greedy_random] Device={device}  seed={args.seed}")
    ...
    run = None if args.no_wandb else wandb.init(
        ...
        name=f"greedy_random_{args.m_name}_s{args.seed}",
        config=vars(args))
    ...
    for it in range(args.num_iters):
        ...
        if iter_sp <= args.target_sparsity * 100 + 0.1:
            print(f"  Target sparsity reached."); break
